...
// constants and masks for the vm
constexpr uint32_t RVM_PAGE_SIZE = 4096 ;
...
/**
 * Opcodes enum defines all the implemented opcodes for the RVM. Limited to 5 bits aka 32 opcodes
*/
enum Opcodes {
	Mov,
	Add,
...
/**
 * RVMCpu struct defines the CPU core which holds registers and pointers to code base, stack base and their respective sizes
*/
struct RVMCpu{
...
typedef struct RVMCpu RVMCpu;
typedef struct Instruction Instruction;

/**
 * \brief RVM is the VM core which will load and run rvm bytecode
 *
 * RVM is the VM core which will load the bytecode, allocate and initialize memory and the CPU.
 * It implements fetching and executing of instructions, stack memory handling and running of the bytecode
*/
class RVM {
public:
	/**
	 * \brief Initializes the RVM from bytecode
	 * @param code Points to the bytecode to be loaded
	 * @param size Holds the size of the bytecode to be loaded
	*/
	RVM(unsigned char* code, uint64_t size);

	/**
	 * Initializes the RVM from bytecode file
...
protected:
	...
	*/
	...
	RVMCpu cpu; /**< Holds the internal state of the CPU */
};