...
RVM::RVM(unsigned char* code, uint64_t size)
{
    ...
    cpu.codeSize = (RVM_PAGE_SIZE * (1 + (size / RVM_PAGE_SIZE)));
    cpu.stackSize = RVM_PAGE_SIZE;

    // allocate memory, code pages, stack, +10 bytes
...
template<class T> inline void RVM::push(T value) {
	// Check bounds
...
template<class T> inline T RVM::pop() {
	// Check bounds
...
bool RVM::execute(Instruction &inst) {
...
bool RVM::fetch(Instruction &inst) const {
